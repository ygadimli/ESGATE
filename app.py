...
GEMINI_API_KEY = "AIzaSyDHJLbfT4k0HSGoqNLfFhn5bGlVmMFYj1g"
...
def sustainability_prediction(current_user, role, user_id):
    ...
    prompt = f"""
    You are an expert ESG analyst. Evaluate a company's sustainability based on:

    Company's Data:
    - Energy Efficiency: {metrics_dict.get('energy_efficiency')} MWh/unit
    - Carbon Intensity: {metrics_dict.get('carbon_intensity')} tCO2e/unit
    - Water Usage: {metrics_dict.get('water_usage')} Liters/unit

    Sector Averages:
    - Energy Efficiency: {sector_averages['energy_efficiency']} MWh/unit
    - Carbon Intensity: {sector_averages['carbon_intensity']} tCO2e/unit
    - Water Usage: {sector_averages['water_usage']} Liters/unit

    Provide a score from 0-10 and analysis. Return only JSON:
    {{
...
def company_summary(current_user, role, user_id):
    ...
    prompt = f"""
    You are a financial and ESG advisor AI. A company has these metrics:

    Overall ESGate Score: {esgatescore}/100
    Interest Rate: {int_rate_percent}
    Default Probability: {default_rate_percent}
    Sustainability Score: {sus_score}/10

    Provide:
    1. A concise summary (2-3 sentences)
    2. Classify risk: Low (<30%), Average (30-45%), High (>50%)
...
def history(current_user, role, user_id):
    ...
    try:
        ...
        if len(history_data) >= 2:
            ...
            prompt = f"""
            Analyze these historical trends for company '{company_name}':
            - Interest Rate changed: {improvements['int_rate_change']:.1f}% (decrease is better)
            - Default Rate changed: {improvements['default_rate_change']:.1f}% (decrease is better)
            - Sustainability Score changed: {improvements['sus_score_change']:.1f}% (increase is better)

            Return JSON with brief comments:
            {{
...
def generate_roadmap(current_user, role, user_id):
    ...
    prompt = f"""
    Sən ESG transformasiya ekspertisən. Aşağıdakı şirkət üçün Avropa requlasiyalarına (EU Taxonomy, CSRD, SFDR) uyğunlaşma yol xəritəsi hazırla.
    
    Şirkət profili:
    - Sektor: {data.get('sector', 'Bilinmir')}
    - Ölçü: {data.get('size', 'Kiçik')}
    - Cari ESG səviyyəsi: {data.get('current_level', 'Başlanğıc')}
    - Hədəf: {data.get('target', 'EU uyğunluğu')}
    - Müddət: {data.get('timeline', '12 ay')}
    ...
    {{
        "phases": [
            {{
                ...
                "title": "Faza adı",
                "duration": "müddət",
                "tasks": ["tapşırıq 1", "tapşırıq 2"],
                "milestones": ["hədəf 1"],
                "resources_needed": "lazım olan resurslar"
            }}
        ],
        "total_investment": "təxmini büdcə",
        "expected_outcomes": ["nəticə 1", "nəticə 2"],
        "key_risks": ["risk 1"],
...
def compare_companies(current_user, role, user_id):
    ...
    selected = [c for c in COMPANIES_DATA if c['id'] in company_ids]
    
    prompt = f"""
    Sən investisiya analitikisən. Aşağıdakı şirkətləri ESG və investisiya perspektivindən müqayisə et.
    
    Şirkətlər: {json.dumps(selected, ensure_ascii=False)}
    
    Cavabını JSON formatında ver:
    {{
        "comparison_summary": "Ümumi müqayisə",
        "best_esg": "Ən yaxşı ESG şirkəti və səbəbi",
        "best_investment": "Ən yaxşı investisiya fürsəti və səbəbi",
        "risk_analysis": "Risk müqayisəsi",
        "recommendation": "Tövsiyə",
        "detailed_analysis": [
            {{"company": "şirkət adı", "strengths": ["güclü tərəf"], "weaknesses": ["zəif tərəf"]}}
        ]
    }}
    ...
    try:
        ...
        if result.endswith('```'):
            result = result[:-3]
        return jsonify({"companies": selected, "analysis": json.loads(result.strip())})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def future_talent_stats(current_user, role, user_id):
    """Get Future Talent ecosystem statistics for investors"""
    stats = {
        "total_kids_enrolled": 1250,
        "tasks_completed": 3420,
        "badges_awarded": 2890,
        "average_financial_literacy": 72,
        "esg_awareness_score": 68,
